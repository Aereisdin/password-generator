...
function button(){
        ...
        var choice = prompt("Please choose the next character in your new password. c for a capital letter, l for a lowercase letter, n for a number or s for a special character.");{
            if(choice === "c"){
            document.getElementById("password").innerHTML = capLetter[Math.floor(Math.random() * capLetter.length)];
             }
             else if(choice === "l"){
            document.getElementById("password").innerHTML = lowLetter[Math.floor(Math.random() * lowLetter.length)];
             }
             else if(choice === "n"){
            document.getElementById("password").innerHTML = number[Math.floor(Math.random() * number.length)];
             }
             else if(choice === "s"){
            document.getElementById("password").innerHTML = special[Math.floor(Math.random() * special.length)];
            }
